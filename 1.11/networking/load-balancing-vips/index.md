...
You can use the layer 4 load balancer by assigning a [VIP](/docs/1.11/networking/load-balancing-vips/virtual-ip-addresses/) in your app definition. After you create a task, or a set of tasks, with a VIP, they will automatically become available to all nodes in the cluster, including the masters.
...
-  You must use a supported [operating system](/docs/1.11/installing/custom/system-requirements/).
...
#### Health checks
Use Mesos health checks. Mesos health checks are surfaced to the load balancing layer. Marathon only converts **command** [health checks](/docs/1.11/deploying-services/creating-services/health-checks/) to Mesos health checks. You can simulate HTTP health checks via a command similar to:
...
- [Assign a VIP to your application](/docs/1.11/networking/load-balancing-vips/virtual-ip-addresses/)
