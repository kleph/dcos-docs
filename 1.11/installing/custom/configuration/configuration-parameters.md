...
| [public_agent_list](#public_agent_list)                               | A YAML nested list (`-`) of IPv4 addresses to your [public agent](/docs/1.11/overview/concepts/#public-agent-node) host names.  |
...
### agent_list
A YAML nested list (`-`) of IPv4 addresses to your [private agent](/docs/1.11/overview/concepts/#private-agent-node) host names.
...
For more information, see the [examples](/docs/1.11/installing/custom/configuration/examples/#docker-credentials).
...
For more information on how these custom checks are used, see the [examples](/docs/1.11/installing/custom/configuration/examples/#custom-checks) and [Node and Cluster Health Check](/docs/1.11/installing/custom/node-cluster-health-check/) documentation.
...
**Important:** Virtual networks require Docker version 1.11 or later. If you are using Docker 1.10 or earlier, you must specify `dcos_overlay_enable: 'false'`. For more information, see the [system requirements](/docs/1.11/installing/custom/system-requirements/).
...
*  `dcos_overlay_enable: 'true'` Enable the DC/OS virtual network. This is the default value. After the virtual network is enabled, you can also specify the following parameters:
    ...
    *  `dcos_overlay_network` This group of parameters defines a virtual network for DC/OS.  The default configuration of DC/OS provides a virtual network named `dcos` with this YAML configuration:
        ...
        *  `overlays`
            *  `name` The canonical name (see [limitations](/docs/1.11/networking/virtual-networks/) for constraints on naming virtual networks).
 ...
 For more information, see the [example](/docs/1.11/installing/custom/configuration/examples/#overlay) and [documentation](/docs/1.11/networking/virtual-networks/).
...
*  `enable_docker_gc: 'true'` Run the docker-gc scripts once every hour. This is the default value for [cloud](/docs/1.11/installing/cloud/) template installations.
*  `enable_docker_gc: 'false'` Do not run the docker-gc scripts once every hour. This is the default value for [custom](/docs/1.11/installing/custom/) installations.
...
For more information, see the [GPU documentation](/docs/1.11/deploying-services/gpu/).
...
### gpus_are_scarce
Indicates whether to treat [GPUs](/docs/1.11/deploying-services/gpu/) as a scarce resource in the cluster.
...
### public_agent_list
A YAML nested list (`-`) of IPv4 addresses to your [public agent](/docs/1.11/overview/concepts/#public-agent-node) host names.
...
-  If you do not have internal hostnames to resolve, you can set this to a public nameserver like Google or AWS. For example, you can specify the [Google Public DNS IP addresses (IPv4)](https://developers.google.com/speed/public-dns/docs/using):
    ...
    - 8.8.8.8
    ```
-  If you do not have a DNS infrastructure and do not have access to internet DNS servers, you can specify `resolvers: []`. By specifying this setting, all requests to non-`.mesos` will return an error. For more information, see the Mesos-DNS [documentation](/docs/1.11/networking/mesos-dns/).
...
See the external persistent volumes [documentation](/docs/1.11/storage/external-storage/) for information on how to create your configuration.
...
*  `use_proxy: 'false'` Do not configure DC/OS [components](/docs/1.11/overview/architecture/components/) to use a custom proxy. This is the default value.
*  `use_proxy: 'true'` Configure DC/OS [components](/docs/1.11/overview/architecture/components/) to use a custom proxy. If you specify `use_proxy: 'true'`, you can also specify these parameters:
...
For more information, see the [examples](/docs/1.11/installing/custom/configuration/examples/#http-proxy).
