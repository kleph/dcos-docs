...
1.  Navigate to the [**Services**](/docs/1.11/gui/#services) tab in the DC/OS web interface.
1.  Select your service, click the vertical ellipsis at the far right, and select **Delete**.

    ![Destroy app](/docs/1.11/img/service-delete.png)
...
For more information, see the [command reference](/docs/1.11/cli/command-reference/#dcos-marathon).
...
From the DC/OS [**Services**](/docs/1.11/gui/) tab, use the JSON editor to add the following as a Marathon task. Replace the values passed to `-r`/`-p`/`-z` according to what needs to be cleaned up.
