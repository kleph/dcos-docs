...
# Service Discovery
  [Service discovery](/docs/1.10/networking/) enables addressing of applications independently of where they are running in the cluster, which is particularly useful in cases where applications may fail and be restarted on a different host.
...
# Mesos-DNS
  ...
  [Mesos-DNS](/docs/1.10/networking/mesos-dns/) assigns DNS entries for every task. The naming pattern for these entries is  *task.scheduler.mesos*
